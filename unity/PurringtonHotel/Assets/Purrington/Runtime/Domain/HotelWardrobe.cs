...
public sealed class WearDefinition {public string id,name,slot,quest,giftNeighbor;public double price;public int giftCat=-1,giftBond=50;public JArray parts=new JArray();}
...
public static class Wardrobe {
 ...
 public static void LoadJson(string json){var items=JObject.Parse(json)["items"] as JArray??throw new FormatException("Wardrobe needs items");var all=items.Select(i=>new WearDefinition{id=(string)i["id"],quest=(string)i["quest"],name=(string)i["name"],slot=(string)i["slot"],price=(double?)i["price"]??0,giftCat=(int?)i["giftCat"]??-1,giftBond=(int?)i["giftBond"]??50,giftNeighbor=(string)i["giftNeighbor"],parts=i["parts"] as JArray??new JArray()}).ToArray();if(all.Any(w=>string.IsNullOrEmpty(w.id)||string.IsNullOrEmpty(w.name)||!Slots.Contains(w.slot)||w.price<0||w.giftCat<-1||w.giftCat>=18||w.giftCat>=0&&(w.giftBond<1||w.giftBond>100)||w.giftNeighbor!=null&&(w.giftCat>=0||w.quest!=null||w.price!=0))||all.Select(w=>w.id).Distinct().Count()!=all.Length)throw new FormatException("Invalid wardrobe item");All=all;}
}
public sealed partial class HotelModel {
 ...
 public bool OwnsWear(string id){var w=Wardrobe.Find(id);if(w==null)return false;if(w.giftNeighbor!=null)return State.hotels.Count>0&&NeighborTierClaimed(w.giftNeighbor)>=3;return w.giftCat>=0?w.giftCat<State.cats.Count&&State.cats[w.giftCat].known&&State.cats[w.giftCat].bond>=w.giftBond:State.wardrobe.Contains(id);}
 public CommandResult BuyWear(string id){var w=Wardrobe.Find(id);if(w==null)return CommandResult.Fail("Choose something from the wardrobe.");if(OwnsWear(id))return CommandResult.Ok(w.name+" is already in the wardrobe.");if(w.quest!=null)return CommandResult.Fail("Accept "+w.quest+" at the clothing store.");if(w.giftNeighbor!=null)return CommandResult.Fail(w.name+" is a best-friend gift from a neighbor.");if(w.giftCat>=0)return CommandResult.Fail(w.name+" is a gift from "+State.cats[w.giftCat].name+" at friendship "+w.giftBond+".");double price=State.settings.godMode?0:w.price;if(State.coins<price)return CommandResult.Fail("Need "+Math.Ceiling(price-State.coins)+" more Cat Coins.");var r=Transaction(()=>{State.coins-=price;State.wardrobe.Add(id);},w.name+" added to the wardrobe");r.cost=r.success?price:0;return r;}
