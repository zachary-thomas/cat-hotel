...
public sealed class GodotCatRig {
 ...
 public GodotCatRig(GodotGeometry geometry,Transform parent,string collection,string id,int seed) {this.seed=seed;Root=geometry.Build(parent,geometry.Recipe(collection,id),Bindings);body=Get("body");head=Get("head");tail=Get("tail");mouth=Get("mouth");for(int i=0;i<4;i++)legs.Add(Get("legs."+i));for(int i=0;i<2;i++){eyes.Add(Get("eyes."+i));ears.Add(Get("ears."+i));}foreach(var t in Bindings.Values)scales[t]=t.localScale;phase=seed*1.71f;thought=Root.GetComponentInChildren<TextMesh>(true);if(thought)thought.text="";}
