...
public sealed class ParityRenderAcceptance:MonoBehaviour {
 ...
 foreach(int map in Enumerable.Range(0,4)){
  app.Model.State.currentHotel=map;app.World.SendMessage("Refresh");app.World.Lighting.Pin(750);
  for(int n=0;n<100;n++){app.Model.Tick(.1f);yield return null;}
  foreach(string suffix in new[]{"opening","neighborhood","evening"}){current=references.OfType<JObject>().First(r=>(string)r["name"]=="map-"+map+"-"+suffix);lockCamera=true;app.World.Lighting.Pin(suffix=="evening"?1170f:750f);yield return null;yield return Capture((string)current["name"]);}
  if(map==0){app.World.Lighting.Pin(750);foreach(string scene in new[]{"neighborhood","fountain","cafe-service"}){current=references.OfType<JObject>().First(r=>(string)r["name"]==scene+"-000");for(int frame=0;frame<30;frame++){app.Model.Tick(.1f);yield return null;yield return Capture(scene+"-"+frame.ToString("000"));}}}
 }
 lockCamera=false;Time.captureFramerate=0;app.Model.State.currentHotel=0;app.World.SendMessage("Refresh");app.World.Lighting.Pin(750);app.World.FocusHotel();app.World.Lighting.Pin(null);app.enabled=true;Application.targetFrameRate=-1;QualitySettings.vSyncCount=0;yield return new WaitForSecondsRealtime(2);
