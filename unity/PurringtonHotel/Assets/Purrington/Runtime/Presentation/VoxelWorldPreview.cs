...
namespace Purrington.Presentation {public sealed partial class VoxelWorld {
 ...
 public void SetCommandPreview(string action,JObject p,bool valid){ClearPreview();preview=Group(renderRoot,"Command preview");string color=valid?"92c39f":"d89487";float x=(float?)p["x"]??0,z=(float?)p["y"]??0;int rotation=(int?)p["rotation"]??0;
 if(action=="paint_path"||action=="erase_path"){foreach(var c in p["cells"]??new JArray())Outline((float)c[0],(float)c[1],1,1,color);return;}
