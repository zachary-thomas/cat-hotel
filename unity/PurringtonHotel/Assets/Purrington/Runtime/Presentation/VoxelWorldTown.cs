...
namespace Purrington.Presentation {
 public sealed partial class VoxelWorld {
  MainStreetArt townArt;GodotCatRig managerRig;
  ...
  public int ActiveStoreInteriorCount=>renderRoot.GetComponentsInChildren<TownInterior>().Count(t=>t.GetComponentsInChildren<Renderer>().Any(r=>r.enabled));
  ...
  public void ExitTownMode(){
   if(!townMode)return;townMode=false;townFollowing=false;pressed=false;
  ...
  void SyncManager(float delta){
   ...
   managerRig.Root.gameObject.SetActive(currentMap==0);
