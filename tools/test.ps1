...
try {
    ...
    foreach ($suite in $Suites) {
        ...
        if ($Rendered -and $suite -in @('test_cat_collision','test_app','test_experience','test_grounds','test_views','test_building','test_startup_offline','test_build_mode','test_build_flow')) { $arguments = @('--path', $projectRoot, '--rendering-method', 'gl_compatibility', '--resolution', $Resolution, '--script', "tests/$suite.gd") }
