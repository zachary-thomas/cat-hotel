...
$existingSuites = @('test_model', 'test_store', 'test_life', 'test_commerce', 'test_audio', 'test_cat_collision', 'test_app', 'test_experience', 'test_grounds', 'test_views', 'test_layout', 'test_layout_grounds', 'test_mobile_layout', 'test_mobile_navigation', 'test_building', 'test_startup_offline')
...
try {
    ...
    foreach ($suite in $Suites) {
        ...
        if ($Rendered -and $suite -in @('test_mobile_navigation','test_cat_collision','test_app','test_experience','test_grounds','test_views','test_building','test_startup_offline','test_build_mode','test_build_flow')) { $arguments = @('--path', $projectRoot, '--rendering-method', 'gl_compatibility', '--resolution', $Resolution, '--script', "tests/$suite.gd") }
