...
func populate(ui, before: Dictionary, after: Dictionary, text_scale: float = 1.0, compact: bool = false) -> void:
	...
	for index in range(3):
		...
		add_child(row)
		var label: Label = ui.label(LABELS[index],roundi((14 if compact else 16)*text_scale))
		label.size_flags_horizontal = Control.SIZE_EXPAND_FILL
		row.add_child(label)
		var numbers: Label = ui.label(str(value) if delta==0 else "%d → %d (%+d)" % [old,value,delta],roundi((14 if compact else 16)*text_scale),ui.GREEN if delta>=0 else Color("a24e49"))
